...
function calculateTax() {
  ...
  if (total_withholding_tax > 0) {
    ...
    // Hide if there's no withholding
    document.getElementById('result_withholding_tax').innerText = '0';
    withholdingTaxParent.style.display = 'block'; 
    // or set to 'none' if you want to hide the line entirely
  }
  ...
  if (X > 0) {
    // "ภาษีที่ต้องชำระจริง" on the left, numeric value on the right
    taxDueReal.innerText = 'ภาษีที่ต้องชำระจริง';
    taxCreditRefund.innerText = formatNumber(X);  // no "บาท"
  } else if (X < 0) {
    // "จำนวนเงินที่คุณต้องขอเครดิตคืน" on the left, numeric value on the right
    taxDueReal.innerText = 'จำนวนเงินที่คุณต้องขอเครดิตคืน';
    taxCreditRefund.innerText = formatNumber(Math.abs(X));  // no "บาท"
  } else {
    // "ดีใจด้วย ท่านไม่ต้องจ่ายภาษี" on the left, nothing on the right
